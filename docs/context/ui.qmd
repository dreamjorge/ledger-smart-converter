...
- Horizontal radio-based navigation (Import / Analytics) with lazy loading
...
# Stable internal keys for navigation
nav_options = {"import": t("nav_import"), "analytics": t("nav_analytics")}
selected_nav = st.radio("Navigation", options=list(nav_options.keys()), horizontal=True, key="nav_page")
...
**Lazy Loading Note**: The app avoids `st.tabs` because it eagerly loads both branches. By using a horizontal radio, the app achieves true lazy-loading of heavy modules (`pandas`, `plotly`, `ml_categorizer`).
