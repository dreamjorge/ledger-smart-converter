...
**Entry function**: `render_analytics_dashboard(*, t, tc, config_dir, copy_feedback_key, ml_engine)`

**Structure**:
- Loads both bank CSVs via `data_service`
...
**Signature**: `render_bank_analytics(df, bank_name, bank_id, t, tc, config_dir, ml_engine)`
...
| Rule Hub | `_render_rule_hub(t, tc, df_filtered, bank_name, bank_id, config_dir, ml_engine)` | inline |
...
6. "Stage Rule Change" → `rulesvc.stage_rule_change()`
7. "Apply Pending Rules" → `rulesvc.merge_pending_rules()` → `ml.train_global_model()` → `st.cache_resource.clear()`
...
- `src/services/data_service.py` - CSV loading
