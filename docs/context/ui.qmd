...
**Location**: `src/ui/` (Streamlit) and `src/ui/flet_ui/` (Flet)

**Pattern**: Service-First Component Rendering

**Entry Points**: 
- Desktop (Flet): `src/flet_app.py`
- Web (Streamlit): `src/web_app.py`

## Flet Desktop Application Structure

### Router & Layout

**File**: `src/flet_app.py` and `src/ui/flet_ui/layout.py`

**Responsibilities**:
- Main desktop window initialization
- Sidebar navigation (`NavigationRail`)
- Routing between `import_view`, `analytics_view`, and `rule_hub_view`
- Global state management (Language selection)

### Core Views
- **Import View** (`import_view.py`): Replaces Streamlit `st.file_uploader` with Flet `FilePicker` for bank statements and PDFs.
- **Analytics View** (`analytics_view.py`): Leverages `ui_service.py` for data formatting. Renders Plotly charts inside Flet containers.
- **Rule Hub View** (`rule_hub_view.py`): AI-assisted categorization correction with fuzzy search.

---

## Streamlit Legacy Application Structure
