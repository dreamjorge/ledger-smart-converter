...
```python
def process_hsbc_cfdi(xml_path: Path, pdf_path: Optional[Path]) -> List[Dict]:
    """Extract transactions from HSBC CFDI files."""
    # 1. Parse XML for structured data
    # 2. If XML missing, fallback to PDF OCR
    # 3. Return list of transaction dicts
```
