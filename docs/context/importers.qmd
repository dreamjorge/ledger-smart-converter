...
The system uses a tiered approach to importing:

1. **Bank-Specific Parsers**: Handlers for unique file formats (e.g., HSBC CFDI XML, Santander LikeU XLSX).
2. **Generic Importer CLI**: A unified entry point (`src/generic_importer.py`) that orchestrates the parsing, normalization, and validation.
3. **PDF Utilities**: Shared logic for OCR and text extraction from statement PDFs.
...
3. Build `raw_description` from parsed source text
4. Build `normalized_description` (deterministic cleanup)
5. Validate transactions
6. Resolve canonical account id from `accounts.yml`
7. Apply rules + ML categorization (normalized text by default; fallback via `LSC_USE_NORMALIZED_TEXT=false`)
8. Write CSV (atomic)
9. Log manifest
