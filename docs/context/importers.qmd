...
```python
def main() -> int:
    """CLI entrypoint for HSBC import flow."""

def extract_movimientos(addenda: ET.Element) -> List[TxnRaw]:
    """Extract TxnRaw entries from Addenda movement nodes."""

def apply_xml_reference_to_pdf(
    pdf_txns: List[TxnRaw],
    xml_txns: List[TxnRaw]
) -> Tuple[List[TxnRaw], Dict[str, Any]]:
    """Merge OCR PDF transactions with XML metadata matches."""
```
