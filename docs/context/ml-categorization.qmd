...
**Feature Flag**: `LSC_USE_NORMALIZED_TEXT` (default enabled)
...
**Model Location**: `settings.config_dir / "ml_models" / "categorizer_v1.joblib"` — uses `settings.config_dir` (not a hardcoded relative path). Default: `config/ml_models/categorizer_v1.joblib`.
**Required Columns**: `destination_name` plus `normalized_description` (preferred) or `description` (fallback)
...
3. Build training frame — text column priority: `normalized_description` (preferred, normalized via `normalize_description()`) → `description` (fallback for older rows)
4. Train `TfidfVectorizer + LogisticRegression`
5. Persist model with `joblib`

**Backfill**: If `normalized_description` is NULL for any row, run `DatabaseService.backfill_normalized_descriptions(normalize_description)` before retraining to maximize coverage of normalized text.

**Retraining Triggers**:
- On **Analytics Dashboard** access if no persisted model is available (lazy-loaded).
...
# For each transaction (input will be normalized internally when feature flag is enabled)
...
- Rule-correction and manual-entry UIs now rely on `services.rules_config_service.load_expense_categories()` for canonical category dropdown values.
- `services.manual_entry_service.load_categories_from_rules()` remains as a thin compatibility wrapper over the shared helper so existing callers and tests keep a stable import path while YAML parsing logic stays centralized.
...
rules:
  ...
  - name: Transport
    any_regex:
      - uber
    set:
      expense: Expenses:Transport:RideShare
...
**Process**:
```python
match_text = normalize_description(raw_description)

for rule in rules_cfg["rules"]:
    for pattern_text in rule["any_regex"]:
        pattern = re.compile(pattern_text, re.IGNORECASE)
        if pattern.match(match_text):
            return rule["set"]["expense"], rule["set"]["tags"]

# No match → try ML prediction
ml_predictions = ml_engine.predict(match_text)
...
**ML Model**:
- Trained on historical Firefly CSV labels (not directly from rules)
- Automated tests for normalized training/prediction paths:
  - `tests/test_ml_categorizer.py`
  - `tests/test_ml_categorizer_normalized.py`
