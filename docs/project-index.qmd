...
- `data_service.py`: DB-first loading with CSV fallback for analytics/UI
...
**UI** (`src/ui/`)
- `flet_app.py`: Desktop application entry point and router
- `web_app.py`: Streamlit web application router
- `pages/`: Streamlit-specific view modules
- `flet_ui/`: Flet-specific view components and layout
- `flet_ui/components.py`: Reusable Flet UI elements
...
Input (PDF/XML/XLSX) → Parse → Validate → Normalize → Categorize (Rules + ML) → Persist/Export
                                                                   ↓
                                               SQLite (`data/ledger.db`) → Analytics Dashboard / Firefly III
```
...
scripts/
├── setup_env.sh           # uv-based environment setup (.venv + requirements install)
├── run_web.sh             # Launch Streamlit from the uv-managed .venv
├── run_flet.sh            # Launch Flet Desktop from the uv-managed .venv
├── run_import.sh          # CLI import helper using the uv-managed .venv
└── generate_dummy_data.py # Generate test data for dashboard

tests/                     # pytest suite (566 collected; 558 fast + 8 slow)
```
...
2. Fall back to CSV when the DB is missing or empty for a bank
3. Aggregate by period/category/bank and display metrics, charts, drill-down

## Reliability Features

- **Validation**: Strict transaction/tag checks with typed errors
- **Atomic writes**: Temp files + rename for CSV outputs
- **Idempotent**: Hash-based deduplication in SQLite
- **Auditable**: Import and rule-change events stored in `audit_events`
- **DB-first reads**: Analytics/UI prefer SQLite and fall back to CSV when needed
...
`setup_env.sh` and `setup_env.ps1` require `uv`, create a repo-local `.venv` via `uv venv`, and install `requirements.txt` via `uv pip install -r`.
...
**Completed**: Validation layer, service architecture, safe rules workflow, SQLite persistence, account mapping, DB pipeline, normalized-description categorization, DB-backed analytics, Firefly export service, audit events, **Flet Desktop UI**, **Family Profiles (multi-user)**, **Manual Entry**, **Batch Deduplication Resolution**.
**Upcoming**: Service-boundary cleanup, configuration contract consolidation, observability improvements, higher critical-path coverage
