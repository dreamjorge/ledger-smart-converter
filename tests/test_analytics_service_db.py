...
def test_calculate_categorization_stats_from_db_returns_empty_stats_for_bank_with_no_rows(tmp_path):
    db_path = tmp_path / "ledger.db"
    db = DatabaseService(db_path=db_path)
    db.initialize()
    db.upsert_account(
    ...
    stats = calculate_categorization_stats_from_db(db_path=db_path, bank_id="santander_likeu")
