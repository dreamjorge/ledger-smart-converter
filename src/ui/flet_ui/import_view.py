...
def get_import_view(page: ft.Page, t: Callable, config: Dict):
    ...
    def _build_dedup_table(duplicate_rows, state, t, page):
        ...
        for row in duplicate_rows[:50]:
            ...
            def make_on_change(hash_key):
                return lambda e: state["dedup_decisions"].update(
                    {hash_key: e.control.value}
                )
            ...
            rows.append(
                ft.DataRow(
                    cells=[
                        ...
                        ft.DataCell(
                            ft.Text(str(row.get("description", ""))[:40], size=12)
                        ),
    ...
    def _quick_start_content() -> ft.Control:
        ...
        tip_key = (
            "tip_santander"
            if state["selected_bank_id"] == "santander_likeu"
            else "tip_hsbc"
        )
    ...
    def handle_process(e):
        if not state["main_file_path"] and not (
            state["force_ocr"] and state["pdf_file_path"]
        ):
        ...
        def _run():
            try:
                ...
                res = imp.run_import_script(
                    ...
                    main_path=Path(state["main_file_path"])
                    if state["main_file_path"]
                    else None,
                    pdf_path=Path(state["pdf_file_path"])
                    if state["pdf_file_path"]
                    else None,
                ...
                if res.returncode == 0:
                    ...
                    new_controls = [
                        ft.Text(
                            f"CSV Generated: {out_csv.name}", color=ft.Colors.GREEN_200
                        ),
                        ft.Text(
                            f"Unknown Merchants: {out_unknown.name}",
                            color=ft.Colors.ORANGE_200
                            if out_unknown.exists()
                    ...
                    if out_csv.exists():
                        try:
                            ...
                            settings = load_settings()
                            dedup_summary, duplicate_rows = (
                                migrate_csvs_to_db_with_dedup(
                            ...
                            new_controls.append(
                                ft.Text(
                                    t("dedup_rows_inserted", n=inserted),
                            ...
                            if duplicate_rows:
                                new_controls.append(
                                    ft.Text(
                                        t("dedup_rows_skipped", n=len(duplicate_rows)),
                                        color=ft.Colors.ORANGE_400,
                                    )
                                )
                                new_controls.append(
                                    _build_dedup_table(duplicate_rows, state, t, page)
                                )
                        except Exception as dedup_exc:
                            new_controls.append(
                                ft.Text(
                                    f"DB sync warning: {dedup_exc}",
                    ...
                    results_col.controls = [
                        ft.Text(res.stderr, color=ft.Colors.RED_200)
                    ]
    ...
    return ft.Column(
        [
            ft.Text(
                t("import_header", bank=get_bank_cfg()["label"]),
                size=32,
            ...
            ft.Row(
                [
                    ft.Dropdown(
                        ...
                        on_select=lambda e: (
                            state.update({"selected_bank_id": e.control.value}),
                            page.update(),
                        ),
                    ),
                ]
                        ...
                        content=ft.Container(
                            content=ft.Column(
                                [
                                    ...
                                    ft.Text(
                                        t("select_xlsx")
                                        if get_bank_cfg()["type"] == "xlsx"
                                        else t("select_xml"),
                                        weight=ft.FontWeight.BOLD,
                                    ),
                                    ft.Button("Browse", on_click=handle_pick_main),
                                    main_file_text,
                                ],
                        ...
                        content=ft.Container(
                            content=ft.Column(
                                [
                                    ...
                                    ft.Button("Browse", on_click=handle_pick_pdf),
                                    pdf_file_text,
                                ],
                                horizontal_alignment=ft.CrossAxisAlignment.CENTER,
                            ),
            ...
            ft.Checkbox(
                ...
                on_change=lambda e: (
                    state.update({"force_ocr": e.control.value}),
                    page.update(),
                ),
            ),
            ...
            results_col,
            ft.Text(
                f"{t('working_dir')}: {root_dir}", size=12, color=ft.Colors.GREY_500
            ),
        ],
        expand=True,
        spacing=20,
